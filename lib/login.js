...
export async function login(username, password) {
  await dbConnect();
  let authorAcc = await authors.findOne({ user: username }).exec();
  if (authorAcc) {  //Cek apakah akun user ada
    ...
    if (isPassValid) {   //Cek apakah pass valid
      ...
      //console.log("authorAcc: " + authorAcc)
      return { authorAcc, cookie: loginKey };
...
export async function addAuthorAccount(email, username, password, name) {
  ...
  if (accCheck1 || accCheck2) {
    ...
    authors.create({ email: email, user: username, pass: hashedPass, name: name }, function (err, author) {
